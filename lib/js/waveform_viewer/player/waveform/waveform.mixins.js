...
define(function () {
  ...
  return {
    ...
    waveformSegmentDrawFunctionTest: function(waveform, id, view, frameStart, segmentStart, segmentLength) {
      if (waveform.segments[id] === undefined){
        return;
      }
      var segment = waveform.segments[id],
        offset_length = segmentLength,
        offset_start = segmentStart - frameStart;
        view.waveformShape.setAttrs({
          x: offset_start,
          width: offset_length
        });
    ...
    waveformKeywordLabelDrawFunction: function(waveform, id, zoomview, view, canvas) {
      if (waveform.segments[id] === undefined){
        return;
      }
      var segment = waveform.segments[id],
      ...
      ctx.lineTo(offset_start, view.height/2); //37
      if ((offset_start + 95) >= canvas.getWidth()) {
        ctx.lineTo(offset_start - 5, view.height/2 - 5);
        ctx.lineTo(offset_start - 5, view.height/2 - 10);
        ctx.lineTo(offset_start - 95, view.height/2 - 10);
        ctx.lineTo(offset_start - 95, view.height/2 + 10);
        ctx.lineTo(offset_start - 5, view.height/2 + 10);
        ctx.lineTo(offset_start - 5, view.height/2 + 5);
      } else {
        ctx.lineTo(offset_start + 5, view.height/2 - 5); //32
        ctx.lineTo(offset_start + 5, view.height/2 - 10); //27
        ctx.lineTo(offset_start + 95, view.height/2 - 10); //27
        ctx.lineTo(offset_start + 95, view.height/2 + 10); //47
        ctx.lineTo(offset_start + 5, view.height/2 + 10); //47
        ctx.lineTo(offset_start + 5, view.height/2 + 5); //42
      }
      ctx.lineTo(offset_start, view.height/2); //37
