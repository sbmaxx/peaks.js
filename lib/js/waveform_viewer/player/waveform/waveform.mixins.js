...
define(function () {
  ...
  return {
    ...
    /**
     * Draw a keyword on overview waveform
     *
     * @param {WaveformData} waveform
     * @param {int} id
     * @param {Kinetic Group} view
     */
    waveformOverviewKeywordDrawFunction: function(waveform, id, overview) {
      if (waveform.segments[id] === undefined){
        return;
      }
      var segment = waveform.segments[id],
        offset_length = segment.offset_length,
        offset_start = segment.offset_start - waveform.offset_start;
        overview.waveformShapeOne.setPoints([offset_start,0,offset_start,10]);
        overview.waveformShapeTwo.setPoints([offset_start,overview.view.height,offset_start,overview.view.height - 10]);
    },
     ...
     * @param {Kinetic Group} view
     */
    waveformZoomviewKeywordDrawFunction: function(waveform, id, zoomview) {
      if (waveform.segments[id] === undefined){
        return;
      }
      var segment = waveform.segments[id],
        offset_length = segment.offset_length,
        offset_start = segment.offset_start - waveform.offset_start,
        xIndex = /*offset_start - (zoomview.circle.getRadius())/2;*/ offset_start - (zoomview.waveformShape.getWidth())/2;
        //circleXIndex = offset_start - (zoomview.circle.getRadius())/2;
      zoomview.waveformShape.setAttrs({
        x: xIndex
      });
     ...
     * @param {Kinetic Group} view
     */
    waveformTagDrawFunction: function(waveform, id, zoomview) {
      if (waveform.segments[id] === undefined){
        return;
      }
      var segment = waveform.segments[id],
      ...
      zoomview.hitRegion.setAttrs({
        x: offset_start - (zoomview.hitRegion.getWidth()/2)
      });
    },

    waveformTagLabelDrawFunction: function(waveform, id, zoomview, canvas) {
      if (waveform.segments[id] === undefined){
        return;
      }
      var segment = waveform.segments[id],
