define([
  ...
  function WaveformOverview(waveformData, $container, options, currentZoomLevel) {
    var that = this;
    that.options = options;
    that.data = waveformData;
    that.$container = $container;
    that.currentZoomLevel = currentZoomLevel;
    ...
    //zoom in when user clicks on the selected segnment
    /*that.refLayer.on("mouseover", function (event) {
      console.log("Activate Zoom", "Current Zoom Level:", peaks.zoom.getZoom(), "Event:", event.type);
      var zoomLevelIndex;
      $(document).on("keypress", function (e) {
        console.log(e.keyCode);
        //Need to get the current zoom level; check if it is good practice to use peaks object from main.js and html page
        if (e.keyCode == 43) { //make sure these are universal key codes?
          console.log("Zoom Out Request");
          //Need to set new zoom level to current zoom level + 1
          //zoomLevelIndex = peaks.currentZoomLevel + 1;
    ...
    /*that.refLayer.on("mouseout", function (event) {
        console.log("Mouseout ref layer");
    });*/
