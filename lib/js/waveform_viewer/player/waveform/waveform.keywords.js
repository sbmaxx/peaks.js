    ...
    */
    var createKeywordWaveform = function(keywordId, word, score, startTime, endTime) {
      ...
      for (var i = 0; i < keywordGroups.length; i++) {
        ...
        if (keywordGroup == keywordZoomGroup) {
          ...
          keywordGroup.rectangle = new Kinetic.Shape({
            drawFunc: function(canvas) {
              mixins.waveformKeywordLabelDrawFunction.call(this, view.data, keyword.id, keywordGroup, view, canvas);
            },
            fill: "#ffff",
            stroke: "black",
            strokeWidth: 1,
        ...
        //Interactions
        keywordGroup.hitRegion.on("mousemove", mOverviewEnter);
    ...
    var updateKeywordWaveform = function (keyword) {
      ...
      if (waveformView.waveformZoomView.data.segments[keyword.id].visible) {
        ...
        keyword.zoom.rectangle.setDrawFunc(function(canvas) {
          mixins.waveformKeywordLabelDrawFunction.call(this, waveformView.waveformZoomView.data, keyword.id, keyword.zoom, keyword.zoom.view, canvas);
        });
      } else {
        keyword.zoom.hide();
      }
