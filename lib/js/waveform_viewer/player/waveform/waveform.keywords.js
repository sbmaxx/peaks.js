...
define([
  ...
  return function (waveformView, keywordData, options) {
    ...
    */
    ...
    /* 
    * Updates each keyword when the zoomview waveform is changed
    *
    ...
    */
    var createKeywordWaveform = function(keywordId, word, score, startTime, endTime) {
      ...
      var mOverviewEnter = function (event) {
        ...
        this.parent.view.tagsLayer.draw();
      };
      ...
      for (var i = 0; i < keywordGroups.length; i++) {
        ...
        if (!view.tagsLayer) {
          view.tagsLayer = new Kinetic.Layer();
          view.stage.add(view.tagsLayer);
          //view.tagsLayer.moveToTop();
        }

        /*
        * Oveview waveform graphic is a rectangle with the keyword text in cased in it and a line indicating 
        * where the keyword is located on the waveform.
        *
        * Zoomview waveform graphic is a rectangle with the keyword text in cased in it.
        */
        //var width =  waveformView.waveformZoomView.width / that.keywordData.length;
        ...
        if (keywordGroup == keywordZoomGroup) {
          ...
          keywordGroup.rectangle = new Kinetic.Shape({
            drawFunc: function(canvas) {
              mixins.waveformTagLabelDrawFunction.call(this, view.data, keyword.id, keywordGroup, canvas);
            },
            fill: "#ffff",
            stroke: "black",
            strokeWidth: 1,
        ...
        view.tagsLayer.add(keywordGroup);
        view.tagsLayer.draw();
      }
    ...
    */
    var updateKeywordWaveform = function (keyword) {
      ...
      if (waveformView.waveformZoomView.data.segments[keyword.id].visible) {
        ...
        mixins.waveformTagDrawFunction(waveformView.waveformZoomView.data, keyword.id, keyword.zoom, width);

        keyword.zoom.rectangle.setDrawFunc(function(canvas) {
          mixins.waveformTagLabelDrawFunction.call(this, waveformView.waveformZoomView.data, keyword.id, keyword.zoom, canvas);
        });
      } else {
        keyword.zoom.hide();
      }

      keyword.zoom.view.tagsLayer.draw();
    };
