...
define([
  ...
  ], function (bootstrap, WaveformData, WaveformOverview, WaveformZoomView, WaveformSegments, WaveformSpeakerSegments, WaveformKeywords) {
    ...
    return {
      init: function (options, ui) {
        ...
        var handleData = function (origWaveformData) {
          ...
          //console.log("Length of Raw Data", origWaveformData.offset_end);
        ...
        xhr.onload = function(response) {
          ...
          if ('XDomainRequest' in window || ('readyState' in this && (this.readyState === 4 && this.status === 200))){
            if ("response" in responseTarget) {
              ...
              handleJSONObject(JSON.parse(xhrData));
            }
          }
        };
        xhr.send(); // Look at it go!

        var handleJSONObject = function (jsonObject) {
          that.speakerJsonObject = jsonObject;

          that.speaker_segments = new WaveformSpeakerSegments(that, that.speakerJsonObject, that.options);
      ...
      getKeywordData: function(options) {
        ...
        var handleJSONObject = function (jsonObject) {
          that.keywordJsonObject = jsonObject;

          that.keywords = new WaveformKeywords(that, that.keywordJsonObject, that.options);
          that.keywords.init();
