...
define([
  ...
  function WaveformZoomView(waveformData, $container, options) {
    ...
    /*that.stage.on("mousedown mousemove mouseup", function (event) {
      $(document).on("keydown", function (e) {
        var isShift = e.shiftKey;
        //SHIFT to start selection process
        if (isShift) {  //Drag and select a segment
          $(that.$container.selector).css("cursor", "crosshair");
          //set the in marker
          that.stage.on("mousedown", function (e) {
            console.log("shift mousedown");
            //get time position of mouse x
            bootstrap.pubsub.emit("setting_in_marker");
          });
          //draw the segment
          that.stage.on("mousemove", function (e) {
            console.log("shift mousemove");
          });
          //set final out marker
          that.stage.on("mouseup", function (e) {
            console.log("shift mouseup");
          });
        }
      });
      $(document).on("keyup", function (e) {
