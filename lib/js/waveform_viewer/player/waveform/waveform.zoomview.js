    ...
    that.stage.on("mouseover", function (event) {
      that.$container.bind('mousewheel', function(e, delta) {
        ...
        if (delta > 0) {
          ...
          if (that.cur_scale >= options.zoomLevels[options.zoomLevels.length - 2]) {
            that.cur_scale = options.zoomLevels[options.zoomLevels.length - 2];
            that.oldZoomLevel = options.zoomLevels.length - 2;
            that.new_zoom_index = options.zoomLevels.length - 2;
        ...
        if ((that.cur_scale < options.zoomLevels[options.zoomLevels.length - 2]) && (that.cur_scale > options.zoomLevels[0])) {
    ...
    bootstrap.pubsub.on("waveform_zoom_level_changed", function (zoom_level, type) {
      ...
      if (zoom_level != that.current_zoom_level) {
        ...
        if (type === "buttons") {
          ...
          if (that.current_zoom_level === peaks.options.zoomLevels.length - 1) {
            if ((that.keywordLayer) && (peaks.waveform.tags.keywordLayerVisibility === true)) {
              that.keywordLayer.setVisible(false);
            }
          }
          that.seekFrame(that.data.at_time(that.currentTime));
        }
