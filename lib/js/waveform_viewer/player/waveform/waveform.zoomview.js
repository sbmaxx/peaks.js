  ...
  function WaveformZoomView(waveformData, $container, options) {
    ...
    /*that.stage.on("mouseover", function (event) {
      that.$container.bind('mousewheel', function(e, delta) {
        ...
        var cur_scale;
        var new_zoom_index;
        //console.log("Zoom Level", that.current_zoom_level);
        if (zoom_direction > 0) {
          //zoom out
          new_zoom_index = that.current_zoom_level + 1
        } else {
          //zoom in
          new_zoom_index = that.current_zoom_level - 1;
        }
        if (delta > 0) {
            ...
            cur_scale = Math.round(that.current_sample_rate + Math.abs(delta / 60));
            for (var i = 0; i < peaks.options.zoomLevels.length; i++) {
              if ((cur_scale >= that.current_sample_rate) && (cur_scale <= peaks.options.zoomLevels[i])) {
                that.current_zoom_level = i;
                break;
              }
            }
        } else {
            //console.log("Zoom In");
            cur_scale = Math.round(that.current_sample_rate - Math.abs(delta / 60));
            for (var x = 0; x < peaks.options.zoomLevels.length; x++) {
              if ((cur_scale <= that.current_sample_rate) && (cur_scale >= peaks.options.zoomLevels[x])) {
                that.current_zoom_level = x;
                break;
              }
            }
        }
        //if (cur_scale <= 4096) {
          ...
          that.current_zoom_level = new_zoom_index;
          that.old_sample_rate = that.current_sample_rate;
          //that.current_sample_rate = cur_scale;
          that.current_sample_rate = peaks.options.zoomLevels[new_zoom_index];
          console.log(that.current_sample_rate);
          //console.log("Delta", delta, "Current Scale", cur_scale, "Zoom Level", that.current_zoom_level);
