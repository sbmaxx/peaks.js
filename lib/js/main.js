...
define([
  ...
  var api = { // PUBLIC API
      ...
      if (!opts.audioElement) {
        ...
        $(api.options.container).html(JST.main).promise().done(function () {
          ...
          bootstrap.pubsub.on("waveformOverviewReady", function () {
            ...
            api.waveform.getSpeakerData(api.options);
            api.waveform.getKeywordData(api.options);
    ...
    segments: {
      ...
      setInMarker: function(startTime) {
        bootstrap.pubsub.emit("setting_in_marker", startTime);
      },

      setOutMarker: function(endTime) {
      ...
      makeSegment: function(selection) {
        api.waveform.speaker_segments.hideZoomGenderSegments(selection);
    ...
    zoom: { // namepsace for zooming related methods
      ...
      showZoomFeedback: function () {
        bootstrap.pubsub.emit("show_current_zoom_level");
      },

      hideZoomFeedback: function () {
        bootstrap.pubsub.emit("hide_current_zoom_level");
      },

      changeZoom: function(zoomLevelIndex) {
        api.zoom.setZoom(zoomLevelIndex);
      },
