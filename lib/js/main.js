  ...
  var api = { // PUBLIC API
    init: function (opts) {
      ...
      if (!opts.audioElement) {
        ...
        api.options = $.extend({
          zoomLevels: [512, 1024, 2048, 4096, 8192, 16384], // Array of scale factors (samples per pixel) for the zoom levels (big >> small)
        ...
        $(api.options.container).html(JST.main).promise().done(function () {
          ...
          bootstrap.pubsub.on("waveformOverviewReady", function () {
            ...
            if (api.options.tagsData) {
              api.waveform.getTagsData(api.options); //Create keyword object
            }
    ...
    segments: {  // namepsace for segment related methods
      ...
      keywordVisibility: function(visible) {
        api.waveform.tags.keywordLayerVisible(visible);
      },
      ...
      makeSegment: function(selection) {
        ...
        api.waveform.speaker_segments.genderLayerVisible(selection);
